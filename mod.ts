...
function writeLE(v: number, buf: Uint8Array, offset: number): void {
  buf[offset] = v & 0xff;
...
export function md5(
  ...
  const state: number[] = md51(msg);
  const buf: Uint8Array = new Uint8Array(BYTES);

  writeLE(state[0], buf, 0);
  writeLE(state[1], buf, 4);
  writeLE(state[2], buf, 8);
  writeLE(state[3], buf, 12);

  return outputEncoding ? decode(buf, outputEncoding) : buf;
}
